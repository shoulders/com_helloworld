...
{   
	...
	public function __construct($config = array())
	{
		...
		$this->view_item = $input->get("view", "project", "string");
	...
	public function cancel($key = null)
	{
		...
		$this->setRedirect(
			...
			Text::_(COM_QWHELLOWORLD_ADD_CANCELLED)
		);
	}
    
	/*
	* Function handing the save for adding a new project record
	...
	*/
	public function save($key = null, $urlVar = null)
	{
		...
		if (!Factory::getUser()->authorise( "core.create", "com_qwhelloworld"))
		{
		...
		*/
		...
		else 
		{
			if ($file['error'] > 0)
			{
				$app->enqueueMessage(Text::sprintf('COM_QWHELLOWORLD_ERROR_FILEUPLOAD', $file['error']), 'warning');
				return false;
			}
			...
			if (!isset($file['name']))
			{
				...
				$app->enqueueMessage(Text::_('COM_QWHELLOWORLD_ERROR_BADFILENAME'), 'warning');
				return false;
			}
			...
			if (File::exists($absolutePathname))
			{
				...
				$app->enqueueMessage(Text::_('COM_QWHELLOWORLD_ERROR_FILE_EXISTS'), 'warning');
				return false;
			}
			...
			{
				// Error in upload
				$app->enqueueMessage(Text::_('COM_QWHELLOWORLD_ERROR_UNABLE_TO_UPLOAD_FILE'));
				return false;
			}
		...
		// notify the administrator that a new project has been added on the front end
		...
		$mailer->setSubject("New project added by " . $current_username);
		$mailer->setBody("New title is " . $validData['title']);
		try 
		{
			$mailer->send(); 
		}
		...
		$this->setRedirect(
				$currentUri,
				Text::_('COM_QWHELLOWORLD_ADD_SUCCESSFUL')
				);
	...
	public function getModel($name = '', $prefix = '', $config = array('ignore_request' => true))
	{
		...
		{
			$input = Factory::getApplication()->input;
			$modelname = $input->get("modelname", "project", "string");
